...
##from requests import Session   ##Replacing by... google request module AuthorizedSession.
...
# Google Services use their own requests module.
# This class is a modified version of BaseRESTAPI to accomodate this. Thanks google.
# https://google-auth.readthedocs.io/en/master/reference/google.auth.transport.requests.html


@dataclass
class GPreparedRequestBaseURLQueueJob:
    prepared_request: requests.PreparedRequest
    prepared_response: requests.Response = None
    failed_attempts: int = 0


class GoogleBaseRESTAPI:
    """ """
    ...
    def __init__(
        ...
        GoogleBaseRESTAPI._create_prepared_requests_baseurl_queue(self.baseurl)
        GoogleBaseRESTAPI._set_period_between_requests(self.baseurl, callrateperhour)
        GoogleBaseRESTAPI._set_geometric_delay_multiplier(
            self.baseurl, geometric_delay_multiplier
        )
        GoogleBaseRESTAPI._set_maximum_geometric_delay_multiplications(
            self.baseurl, maximum_geometric_delay_multiplications
        )
        GoogleBaseRESTAPI._set_maximum_failed_attempts(
            self.baseurl, maximum_failed_attempts
        )
        GoogleBaseRESTAPI._set_request_timeout(self.baseurl, request_timeout)
        GoogleBaseRESTAPI._startapiticker(self)
    ...
    def get_request(self, endpoint, **kwargs):
        ...
        GoogleBaseRESTAPI.prepared_requests_baseurl_queues[self.baseurl].put(
    ...
    def post_request(self, endpoint, **kwargs):
        standard_session, auth_headers = self.get_session()
        url = f"{self.baseurl}{endpoint}"
        myrequest = Request("POST", url, headers=auth_headers, **kwargs)
    ...
    def put_request(self, endpoint, **kwargs):
        standard_session, auth_headers = self.get_session()
        url = f"{self.baseurl}{endpoint}"
        myrequest = Request("PUT", url, headers=auth_headers, **kwargs)
        prepared_request = standard_session.prepare_request(myrequest)
    ...
    def delete_request(self, endpoint, **kwargs):
        standard_session, auth_headers = self.get_session()
        url = f"{self.baseurl}{endpoint}"
        myrequest = Request("DELETE", url, headers=auth_headers, **kwargs)
    ...
    def head_request(self, endpoint, **kwargs):
        standard_session, auth_headers = self.get_session()
        url = f"{self.baseurl}{endpoint}"
        myrequest = Request("HEAD", url, headers=auth_headers, **kwargs)
    ...
    def options_request(self, endpoint, **kwargs):
        standard_session, auth_headers = self.get_session()
        url = f"{self.baseurl}{endpoint}"
        myrequest = Request("OPTIONS", url, headers=auth_headers, **kwargs)
    ...
    def _wait_for_response(self, preparedrequestbaseurlqueuejob):
        ...
        place_in_queue = (
            GoogleBaseRESTAPI.prepared_requests_baseurl_queues[self.baseurl].qsize() + 1
        )
        ...
        time_in_seconds_to_wait = (
            periods_to_wait
            * GoogleBaseRESTAPI.baseurl_period_between_request[self.baseurl]
        )
        ...
        delta = 0.01
        delta_value = (
            delta * GoogleBaseRESTAPI.baseurl_period_between_request[self.baseurl]
        )
    ...
    def google_auth(self):
        credentials, project = google.auth.default(scopes=self.scopelist)
    ...
    def _set_period_between_requests(cls, baseurl, callrateperhour):
        if baseurl in GoogleBaseRESTAPI.baseurl_period_between_request:
            return GoogleBaseRESTAPI.baseurl_period_between_request[baseurl]
        else:
            GoogleBaseRESTAPI.baseurl_period_between_request[baseurl] = float(
                3600 / callrateperhour
            )

    @classmethod
    def _set_maximum_failed_attempts(cls, baseurl, maximum_failed_attempts):
        if baseurl in GoogleBaseRESTAPI.baseurl_maximum_failed_attempts:
            return GoogleBaseRESTAPI.baseurl_maximum_failed_attempts[baseurl]
        else:
            GoogleBaseRESTAPI.baseurl_maximum_failed_attempts[
                baseurl
            ] = maximum_failed_attempts

    @classmethod
    def _set_request_timeout(cls, baseurl, request_timeout):
        if baseurl in GoogleBaseRESTAPI.baseurl_request_timeout:
            return GoogleBaseRESTAPI.baseurl_request_timeout[baseurl]
        else:
            GoogleBaseRESTAPI.baseurl_request_timeout[baseurl] = request_timeout

    @classmethod
    def _set_geometric_delay_multiplier(cls, baseurl, geometric_delay_multiplier):
        if baseurl in GoogleBaseRESTAPI.baseurl_geometric_delay_multiplier:
            return GoogleBaseRESTAPI.baseurl_geometric_delay_multiplier[baseurl]
        else:
            GoogleBaseRESTAPI.baseurl_geometric_delay_multiplier[
                baseurl
    ...
    def _set_maximum_geometric_delay_multiplications(
        ...
        if baseurl in GoogleBaseRESTAPI.baseurl_maximum_geometric_delay_multiplications:
            return GoogleBaseRESTAPI.baseurl_maximum_geometric_delay_multiplications[
                baseurl
            ]
        else:
            GoogleBaseRESTAPI.baseurl_maximum_geometric_delay_multiplications[
                baseurl
    ...
    def _startapiticker(cls, self):
        ...
        cls.prepared_requests_baseurl_tickers[self.baseurl] = self.GAPITicker(
            self.get_session()[0],
            ...
            GoogleBaseRESTAPI.baseurl_maximum_failed_attempts[self.baseurl],
            GoogleBaseRESTAPI.baseurl_geometric_delay_multiplier[self.baseurl],
            GoogleBaseRESTAPI.baseurl_request_timeout[self.baseurl],
        )

    class GAPITicker:
        """
        The GAPITicker starts up the job queue and waits for new jobs that need to be processed.
        """
